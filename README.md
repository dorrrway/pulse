...
- 在完整面板状态卡片下方新增“开机时长”横条，基于系统上次启动时间显示本次开机已持续多久。
- 收紧完整面板底部控制栏上下留白，使底部按钮区域更紧凑。
- 完整面板状态卡片在悬停提示和辅助功能中提供完整说明，内存压力英文明细使用更短文案以减少截断。
- 优化完整面板状态卡片：内存压力、温度、电源和磁盘 I/O 的像素块按状态或活动强度变色，更新温度与电源文案，并在 CPU 占用排行中对高占用进程使用警示色。
