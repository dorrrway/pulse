...
- 截图完成提醒新增“编辑”入口，会打开类似钉图的前台悬浮编辑窗口，在图片下方提供独立功能区，支持马赛克遮挡、矩形、圆形和箭头，并在完成后把编辑后的图片重新写回系统剪贴板。
- 钉到前台的截图悬浮窗支持拖拽边缘或四角按原图比例缩放，四角缩放会锁定本次拖拽方向以保持画面稳定，仍保持无边框图片窗口样式。
...
- 新增“蓝牙”模块，以列表显示本机蓝牙设备、连接状态和可读取的电量；键盘、触控板、鼠标等单电量设备显示一个环状电量，AirPods 类设备在 macOS 返回时分别显示左耳、右耳和盒子电量，并可对已配对设备发起连接或断开。
