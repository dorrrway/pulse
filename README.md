...
Pulse 会读取本机系统指标，例如 CPU、内存、磁盘、网络收发字节计数、电池、热状态、磁盘 I/O 和系统上次启动时间。为显示占用排行，Pulse 也会读取本机正在运行进程的名称、CPU/内存占用，以及用于显示 App 图标的本机 App bundle 路径。为显示“应用程序”列表，Pulse 会读取标准应用位置中的 App 名称、bundle id、版本和 bundle 路径，并读取这些 App 的运行状态；如果用户将 App 加入“常用应用”，Pulse 会在本机保存对应 App bundle 路径。用户从“应用程序”模块主动选择“卸载并清理”时，Pulse 会在本机扫描该 App bundle 和用户 Library 中常见的应用支持文件、容器、偏好设置、缓存、日志、登录代理、保存状态、WebKit 和 HTTP 存储路径，展示完整候选路径供确认，并且只会把用户确认选择的项目移到系统废纸篓。授予蓝牙权限后，蓝牙模块会在本机内存中读取蓝牙设备名称、类型、连接状态、蓝牙地址和 macOS 暴露的电量与充电状态字段，用于显示列表、连接/断开已配对设备、展示可读取的电量和充电状态，并在设备电量过低时显示本机提醒；这些信息不上传、不写入持久数据库，也不保存电量或充电历史。通知建议层只根据上述本机系统指标和蓝牙电量在界面中生成建议；用户关闭建议面板或单条建议时，Pulse 只在本机偏好设置保存开关和建议 ID，不保存原始指标历史。剪贴板模块会读取系统剪贴板变化，并把文本、链接、文件 URL/路径表示、图片、原始 pasteboard 表示、来源 App 线索和敏感/临时 marker 保存在本机历史中；复制文件时 Pulse 不主动读取或保存文件本体内容。敏感内容默认遮罩，但仍可搜索和复制。双击剪贴板记录可在获得 macOS 辅助功能权限后向当前聚焦位置发送一次粘贴快捷键；Pulse 不读取目标 App 的输入内容。OCR 默认关闭，开启后只在本机用 Vision 处理图片文字。备忘模块会把用户主动输入的备忘、待办状态、置顶状态和时间戳保存在本机 Application Support 目录中，不上传。

截屏录屏模块只在用户点击或触发快捷键时调用 macOS 原生截图与录屏能力；首次使用需要授予 macOS“屏幕录制”权限。截图继续使用原生 `screencapture`，默认会在截图前临时隐藏 Pulse；全屏截图会先显示本机显示器选择层，用户选择某个显示器后只截取该显示器；截图会写入系统剪贴板，如果剪贴板历史开启，该图片会按现有剪贴板规则保存在本机历史中。录屏开始前，全屏录屏会显示本机显示器选择层并直接以该显示器作为 ScreenCaptureKit 目标；窗口录屏会复用 macOS 原生截图选择器创建并立即删除一张临时选择截图，仅用于读取系统写入的窗口元数据；自定义区域录屏会显示本机 AppKit 选择层，让未选区域暗下去并保持所选区域原始亮度。录制开始后，自定义区域会继续显示一个不接收鼠标事件的本机区域指示层，保持未录制区域暗化和选区蓝色边界；Pulse 会从最终录制输出中排除该指示层。随后使用 macOS 原生 ScreenCaptureKit 录制选中的显示器、窗口或区域，并通过 AVFoundation 写入临时 `.mov` 文件。

录屏不采集系统音频或麦克风音频，默认隐藏鼠标指针。录制开始后 Pulse island 会恢复显示录制计时和停止按钮；开启“隐藏 pulse 界面”时，ScreenCaptureKit 会从最终录制输出中排除 Pulse 自身，因此保存后的视频不会显示 Pulse island。macOS 录制期间显示的系统共享指示由系统控制，Pulse 不会把它写入最终视频。停止后，录屏文件会先写入系统临时目录，并在 island 中显示视频预览；用户可先用本机播放器预览 `.mov`，再选择保存、分享、关闭或丢弃，关闭、丢弃或替换预览会删除临时文件。
...
- 蓝牙面板现在会在系统蓝牙关闭时显示顶部提醒，并提供开启蓝牙的系统设置入口，避免误显示为没有设备。
- 岛形 header 和主面板之间新增通知建议层，可显示并关闭磁盘空间、电量、温度、内存压力和蓝牙电量建议；多条建议会在同一水平队列中保留一条完整卡片，其余收缩为圆形图标，点击图标会用线性动画在原位置展开并切换到对应面板，也可在设置中整体关闭。
- “应用程序”模块新增右键“卸载并清理”普通确认窗口，可扫描 App bundle 和本机 Library 常见残留路径，用户确认后只把所选项目移到系统废纸篓。
- “卸载并清理”会提前说明 macOS 可能请求 App 管理或其他 App 数据权限；确认流程不再挂在岛形面板内，打开卸载窗口后会收起岛形面板，避免系统授权弹窗被岛形层级遮挡。遇到权限拒绝时会显示中文原因、App 管理和完全磁盘访问入口，并允许授权后只重试失败项。
- 全屏截图和全屏录屏现在先选择目标显示器，多显示器环境下不会再误进入窗口选择；窗口录屏继续使用 macOS 原生窗口选择器。
- 截图编辑器现在会在当前屏幕居中打开，并在进入编辑时收起 island 预览，避免小图被顶部面板遮挡。
- 发布包：[Pulse-2.5.0.dmg](https://github.com/dorrrway/pulse/releases/download/v2.5.0/Pulse-2.5.0.dmg)；SHA-256：`38da615c20d487aa5d1bb8929420b53015af662fa860ac4874376d611dcaa612`。
...
- 三种录屏模式也可在截屏录屏面板和设置中设置全局快捷键；快捷键会遵循“隐藏 pulse 界面”和“隐藏鼠标”设置，正在录制同一模式时再次触发会停止录制并进入 island 视频预览，用户可先播放确认，再保存、分享、关闭或丢弃临时 `.mov`。
- 截图和视频预览右上角改为关闭按钮，可直接退出预览；从展开面板进入的截图预览关闭后会回到展开状态，关闭视频预览会删除未保存的临时 `.mov`。
...
- 新增“备忘”模块，支持在 Pulse island 中通过更大的整行输入栏按回车创建备忘、右键设为待办，并可按全部/备忘/待办/完成筛选、搜索、编辑、置顶、复制、删除和清理已完成待办；所有内容只保存在本机。
- 发布包：[Pulse-2.4.0.dmg](https://github.com/dorrrway/pulse/releases/download/v2.4.0/Pulse-2.4.0.dmg)；SHA-256：`5937825a99e43f2fbf599cd2efb9d871ba16e339bac1b73a504db553c2e3bdae`。
...
- 优化截图编辑器文字工具：输入时使用透明且居中对齐的编辑框，点击画布空白区域即可完成输入，已添加的文字可拖动调整位置。
